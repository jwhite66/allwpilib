...
static void decode_ds_tcp_packet(uint8_t* packet, int len) {
  // TODO - implement decode...
  std::cerr << "I got a golden packet! (TCP packet not handled yet)" << len
            << std::endl;
...
static void send_reply_packet(int socket, struct sockaddr* addr,
                              socklen_t addrlen, int reply_port) {
    ...
    while (true) {
      ...
      do {
        ...
        if (rc > 0) {
          ...
          if (len > 0) decode_ds_tcp_packet(buf, len);
          break;
        }
      } while (true);

      close(client);
      sleep(1);  // TODO - get a fractional sleep
    }
  }
}
