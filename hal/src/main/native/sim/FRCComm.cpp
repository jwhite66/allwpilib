...
#if defined(WIN32) || defined(_WIN32)
  sendto(socket, reinterpret_cast<const char *>(data), sizeof(data), 0, addr, addrlen);
#else
...
static void udp_thread_func() {
  while (true) {
    ...
    do {
      ...
      int rc = recvfrom(socket, reinterpret_cast<char *>(buf), sizeof(buf), 0, &addr, &addrlen);
#else
