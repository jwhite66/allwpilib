...
int DSCommPacket::DecodeTCP(uint8_t* packet, int len) {
  ...
  Lock();
  // TODO(jwhite@codeweavers.com) - implement decode once we have a packet we need
  ...
  for (i = 0; i < cMaxJoysticks; i++) {
    ...
    if (i < m_joysticks.size()) {
      ...
      for (j = 0; j < axes.count; j++)
        axes.axes[j] = m_joysticks[i].axes[j];
...
void DSCommPacket::SendToHALSim(void) {
  ...
  if (m_udp_packets == 1) {
      ...
      std::chrono::duration<double> delta = (now - m_time_at_start);
      match_time = delta.count();
  }
  SendJoysticks();
  Unlock();

  HALSIM_SetDriverStationMatchTime(match_time);
