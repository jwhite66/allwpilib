...
static void TCPThreadFunc(DSCommPacket *ds) {
  while (true) {
    int socket = OpenListenSocket(1740, true);
    if (socket < 0) {
#if defined(WIN32) || defined(_WIN32)
      Sleep(1000);
#else
...
static void UDPThreadFunc(DSCommPacket *ds) {
  while (true) {
    int socket = OpenListenSocket(1110, false);
    if (socket < 0) {
#if defined(WIN32) || defined(_WIN32)
      Sleep(1000);
#else
    ...
    do {
      ...
      if (rc > 0) {
        ds->DecodeUDP(buf, rc);
        SendReplyPacket(socket, &addr, addrlen, 1150, ds);
        ds->SendToHALSim();
      } else {
        break;
      }
...
int HALSIM_InitExtension(void) {
  ...
  if (once) {
    std::cerr << "Error: cannot invoke HALSIM_InitExtension twice." << std::endl;
    return -1;
  }
  once = true;
