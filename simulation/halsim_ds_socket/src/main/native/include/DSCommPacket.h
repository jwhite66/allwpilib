...
class DSCommPacket {
 public:
  DSCommPacket(void) { std::fill_n(m_joystick_types, cMaxJoysticks, -1); }
  ...
  static const uint8_t cJoystickNameType = 0x02;
 ...
 private:
  ...
  std::string m_joystick_names[cMaxJoysticks];
  int m_joystick_types[cMaxJoysticks];
  std::mutex m_mutex;
  int m_udp_packets;
